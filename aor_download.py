"""Annals of Operations Research (ISSN 0254-5330) downloader, 2018-2026, via JMI/ONOS IP.
Springer = fastest publisher: direct curl of link.springer.com/content/pdf/<DOI>.pdf
(no browser). Resume-safe by disk, time-boxed. Files: aor_downloads/V<vol>I<iss>/<doi>.pdf
"""
...
DATA = json.loads((PROJECT/"aor_dois.json").read_text(encoding="utf-8"))
DL = PROJECT/"aor_downloads"; DL.mkdir(exist_ok=True)
CONC = 16
...
def main(budget):
    ...
    print(f"AOR 2018-2026: target={total}, on_disk={have0}, queue={len(work)}, conc={CONC}, budget={budget}s", flush=True)
    if not work:
