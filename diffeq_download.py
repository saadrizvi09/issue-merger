"""Differential Equations (ISSN 0254-5330) downloader, 2018-2026, via JMI/ONOS IP.
Springer = fastest publisher: direct curl of link.springer.com/content/pdf/<DOI>.pdf
(no browser). Resume-safe by disk, time-boxed. Files: diffeq_downloads/V<vol>I<iss>/<doi>.pdf
"""
...
DATA = json.loads((PROJECT/"diffeq_dois.json").read_text(encoding="utf-8"))
DL = PROJECT/"diffeq_downloads"; DL.mkdir(exist_ok=True)
CONC = 16
...
def download(item):
    ...
    for attempt in range(4):
        try:
            ...
            if valid_pdf(fp): return (item, True, fp.stat().st_size)
...
def main(budget):
    work = build_work(); total = DATA["total_articles"]; have0 = on_disk()
    print(f"DiffEq 2018-2026: target={total}, on_disk={have0}, queue={len(work)}, conc={CONC}, budget={budget}s", flush=True)
    if not work:
    ...
    try:
        for fut in as_completed(futs):
            ...
            if n % 25 == 0 or not good:
                el = time.time()-t0
                print(f"  [{have0+ok}/{total}] {item[0][-20:]} {'OK '+str(sz//1024)+'KB' if good else 'FAIL'} {n/el:.2f}/s ({el:.0f}s)", flush=True)
