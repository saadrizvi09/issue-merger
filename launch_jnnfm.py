"""Launch run_jnnfm_loop.py as a detached Windows process (survives terminal close)."""
import subprocess, sys
from pathlib import Path

PROJECT = Path("C:/Projects/Automate pdf merge journal")
PID_FILE = PROJECT / "jnnfm_loop.pid"
LOG_FILE = PROJECT / "jnnfm_loop.log"
...
def main():
    ...
    proc = subprocess.Popen([sys.executable, "-u", str(PROJECT / "run_jnnfm_loop.py")],
                            cwd=str(PROJECT), stdout=log, stderr=subprocess.STDOUT,
                            creationflags=subprocess.DETACHED_PROCESS | subprocess.CREATE_NO_WINDOW)
    PID_FILE.write_text(str(proc.pid))
    print(f"Launched detached JNNFM loop PID {proc.pid} — log: {LOG_FILE}")
